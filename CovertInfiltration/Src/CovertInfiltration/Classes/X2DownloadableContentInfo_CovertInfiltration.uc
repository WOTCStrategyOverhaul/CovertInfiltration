...
static event OnPostTemplatesCreated()
{
	...
	Template = X2StaffSlotTemplate(StrMgr.FindStrategyElementTemplate('CovertActionScientistStaffSlot'));
	Template.IsUnitValidForSlotFn = IsUnitValidForCovertActionScientistSlot;

	Template = X2StaffSlotTemplate(StrMgr.FindStrategyElementTemplate('CovertActionEngineerStaffSlot'));
	Template.IsUnitValidForSlotFn = IsUnitValidForCovertActionScientistSlot;
}

static function bool IsUnitValidForCovertActionScientistSlot(XComGameState_StaffSlot SlotState, StaffUnitInfo UnitInfo)
{
...
exec function OpenDebugStrategy()
{
	local UIFInalShell Screen;

	Screen = UIFInalShell(`SCREENSTACK.GetCurrentScreen());
	if (Screen != none)
	{
		`XCOMHISTORY.ResetHistory();
		XComShellPresentationLayer(Screen.Owner).UIStrategyShell();
	}
}
